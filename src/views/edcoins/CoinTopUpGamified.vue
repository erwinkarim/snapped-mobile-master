  ...
  <dashboard-layout class="pt-5" :has-fixed-header="true" :has-custom-bottom-bar="true">
    ...
    <template v-slot:content>
      ...
      <div class="px-5 w-full md:px-10">
        <div class="mt-2 mb-3">
          <div class="overflow-hidden justify-between py-3 px-3 max-w-sm md:max-w-xl h-full rounded rounded-xl bg-gray-secondary">
            <div class="flex flex-row items-center h-full text-left text-purple-primary" :to="{ name:'coin.top_up_buy' }">
              You have X coins now. You can request to transfer Y coins to Gamified
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="px-5 w-full md:px-10">
        ...
        <div>
          <button @click="requestTopUp" class="border px-5">Request Transfer</button>
        </div>
      </div>
    </template>
